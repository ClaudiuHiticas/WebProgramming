...
namespace Nou
{
    ...
    public partial class home : System.Web.UI.Page
    {
        ...
        {
            String idSes = Session["id"].ToString();
            ...
            table += "<tr><th>Id</th><th>UserId</th><th>Name</th><th>Description</th><th>Value</th></tr>";
            foreach (DataRow dr in dt.Rows)
            {
                String userId = dr["userId"].ToString();
                if(userId == idSes)
                {
                    int val = (int)dr["value"];
                    if (val > 10)
                    {
        ...
        protected void addFileFunc(object sneder, EventArgs e)
        {
            ...
            String descriptionV = description.Text;
            String valueV = value.Text;
            String idSes = Session["id"].ToString();

            con.Open();
            MySqlCommand cmd = con.CreateCommand();
            cmd.CommandType = CommandType.Text;
            cmd.Parameters.AddWithValue("@userId", idSes);
            cmd.Parameters.AddWithValue("@name", nameV);
            cmd.Parameters.AddWithValue("@description", descriptionV);
            cmd.Parameters.AddWithValue("@value", valueV);
